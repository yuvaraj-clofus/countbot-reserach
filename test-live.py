...
# ── Inference backend ─────────────────────────────────────────────────────────
# Set USE_TRT = True  → TensorRT FP16 engine  (~15-25 fps on Jetson Orin Nano)
# Set USE_TRT = False → PyTorch               (~3-5  fps on Jetson Orin Nano)
USE_TRT         = True
TRT_ENGINE_PATH = "/home/vikbot/Documents/countbot/convert-trt/dinov2_vits14.engine"
...
# ── Patch grid ────────────────────────────────────────────────────────────────
STRIP_X_FRACTION = 0.55       # counting line at 85% of frame width
...
NUM_STRIPS_PER_LINE = 1       # strips per line group (start and end)
...
class TRTDino:
    """
    TensorRT FP16 drop-in replacement for torch.hub DINOv2.
    Mimics forward_features() used by _forward_patch_tokens().
    """
    def __init__(self, engine_path, device="cuda"):
        ...
        self._output_cache = {}
        # dinov2_vits14: 224/14=16 → 16×16=256 tokens, embed_dim=384
    ...
    def forward_features(self, x: torch.Tensor) -> dict:
        """x: (B,3,224,224) float32 CUDA → {"x_norm_patchtokens": (B,256,384)}"""
        x = x.contiguous()
        B = x.shape[0]
        self.context.set_input_shape("images", (B, 3, 224, 224))
        out = self._output_cache.get(B)
        if out is None or out.device != torch.device(self.device):
            out = torch.empty((B, self._num_tokens, self._embed_dim),
                              dtype=torch.float32, device=self.device)
            self._output_cache[B] = out
        # TensorRT 10 uses set_tensor_address + execute_async_v3
        self.context.set_tensor_address("images",       x.data_ptr())
        self.context.set_tensor_address("patch_tokens", out.data_ptr())
        stream = torch.cuda.current_stream(device=x.device).cuda_stream
        self.context.execute_async_v3(stream_handle=stream)
        return {"x_norm_patchtokens": out}


def load_dino(device):
    if USE_TRT and device == "cuda":
        if not os.path.exists(TRT_ENGINE_PATH):
            print(f"[WARN] TRT engine not found at: {TRT_ENGINE_PATH}")
            print("[WARN] Falling back to PyTorch. Build the engine first:")
            print("       python3 export_dino_onnx.py")
            print("       trtexec --onnx=dinov2_vits14.onnx --saveEngine=dinov2_vits14.engine \\")
            print("               --fp16 --memPoolSize=workspace:4096M --skipInference")
        else:
            print("[DINO] Backend: TensorRT FP16")
            return TRTDino(TRT_ENGINE_PATH, device)
    elif USE_TRT:
        print(f"[WARN] TensorRT requested but device is {device}; falling back to PyTorch")
    # ── PyTorch fallback ──────────────────────────────────────────────────────
    print("[DINO] Backend: PyTorch")
...
class ReferenceBank:
    ...
    def load(self, path, device, num_patches=None):
        ...
        if (data["strip_width"] != STRIP_WIDTH or
            ...
            data.get("embedding_mode") != EMBEDDING_MODE or
            (num_patches is not None and data.get("num_patches") != num_patches)):
            return False
...
def build_display(frame, strip_a, strip_b, patch_sims, triggered, thresholds,
                  ...
                  triggered_end=None, triggered_start=None,
                  fps_value=None, backend_label=""):
    """
    ...
    # Strip band colors: normal (cyan/yellow) → red when any patch triggered on that line.
    end_any   = triggered_end   is not None and any(triggered_end)
    start_any = triggered_start is not None and any(triggered_start)
    end_color   = (0, 0, 255)   if end_any   else (0, 220, 220)
    start_color = (0, 0, 255)   if start_any else (220, 220, 0)

    # Draw strip zones as translucent colored bands + outlines + labels.
    overlay_strips = vis.copy()
    for x in end_positions:
        cv2.rectangle(overlay_strips, (x, 0), (x + STRIP_WIDTH, H - 1), end_color, -1)
    for x in start_positions:
        cv2.rectangle(overlay_strips, (x, 0), (x + STRIP_WIDTH, H - 1), start_color, -1)
    cv2.addWeighted(overlay_strips, 0.22, vis, 0.78, 0, vis)
    for si, x in enumerate(end_positions):
        cv2.rectangle(vis, (x, 0), (x + STRIP_WIDTH, H - 1), end_color, 2)
        cv2.putText(vis, f"E{si+1}", (x + 2, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.45, end_color, 2)
    for si, x in enumerate(start_positions):
        cv2.rectangle(vis, (x, 0), (x + STRIP_WIDTH, H - 1), start_color, 2)
        cv2.putText(vis, f"S{si+1}", (x + 2, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.45, start_color, 2)

    # Highlight triggered patches per line independently (like patch_counter_v2 per-strip).
    if triggered_end is not None or triggered_start is not None or triggered is not None:
        overlay = vis.copy()
        trig_e = triggered_end   if triggered_end   is not None else ([False] * num_patches)
        trig_s = triggered_start if triggered_start is not None else ([False] * num_patches)
        for i in range(num_patches):
            y0 = i * patch_height
            y1 = y0 + patch_height
            if trig_e[i]:
                for x in end_positions:
                    cv2.rectangle(overlay, (x, y0), (x + STRIP_WIDTH, y1), (0, 0, 255), -1)
            if trig_s[i]:
                for x in start_positions:
                    cv2.rectangle(overlay, (x, y0), (x + STRIP_WIDTH, y1), (0, 0, 255), -1)
    ...
    fps_txt = f"FPS: {fps_value:.2f}" if fps_value is not None else "FPS: --"
    if backend_label:
        fps_txt = f"{backend_label}  {fps_txt}"
    cv2.putText(vis, fps_txt, (18, 128), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 3)
    cv2.putText(vis, fps_txt, (18, 128), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
...
def main():
    ...
    backend_label = "TRT" if isinstance(model, TRTDino) else "PT"
    import time
    ...
    if bank.exists(bank_path):
        if not bank.load(bank_path, device, num_patches=num_patches):
            print("  [TRAIN] Config mismatch (resolution changed?), rebuilding...")
            bank = ReferenceBank()
    ...
    cv2.resizeWindow("PatchCount", 640, 480)
    ...
    while True:
        if not paused:
            ...
            if key == ord('q'):
                ...
                last_sims = None
                last_triggered = None
                last_triggered_end = last_triggered_start = None
                ...
                disp = build_display(frame, last_end_strip, last_start_strip,
                                     ...
                                     last_triggered_end, last_triggered_start,
                                     last_fps, backend_label)
        ...
        if frame_n % PROCESS_EVERY_N == 0 or last_sims is None:
            ...
            max_sims, triggered_tensor, trig_end_tensor, trig_start_tensor, display_thresholds_tensor = bank.compare(
                end_embs, start_embs, CONSENSUS_MIN_STRIPS
            )
            max_sims_display = max_sims.detach().cpu().numpy()
            display_thresholds_display = display_thresholds_tensor.detach().cpu().numpy()
            triggered = triggered_tensor.detach().cpu().tolist()
            triggered_end = trig_end_tensor.detach().cpu().tolist()
            triggered_start = trig_start_tensor.detach().cpu().tolist()
            sync_for_timing()
            ...
            last_sims = max_sims_display
            last_triggered = triggered
            last_triggered_end = triggered_end
            last_triggered_start = triggered_start
            last_end_strip = end_strips[0] if end_strips else None
            last_start_strip = start_strips[0] if start_strips else None
            display_thresholds = display_thresholds_display
        ...
        if disp_w > max_w:
            scale = max_w / disp_w
            disp = cv2.resize(disp, (max_w, int(disp_h * scale)))
        cv2.imshow("PatchCount", disp)
