...
"""
video
python3 dinodiff-live-v2-cl.py --rotate 0   --entry-line 20,20,20,75 --capture-line 50,20,50,75 --exit-line 80,20,80,75 --video count2.mp4 

live
python3 dinodiff-live-v2-cl.py --rotate 90   --entry-line 20,20,20,75 --capture-line 50,20,50,75 --exit-line 80,20,80,75
...
# Three-line counting — fractions of frame width
LINE_ENTRY_FRAC   = 0.3   # entry line (objects must cross this first)
LINE_CAPTURE_FRAC = 0.55  # capture line (snapshot taken here, held in memory)
LINE_EXIT_FRAC    = 0.8   # exit line  (crossing here saves snapshot + counts)
...
def parse_args():
    ...
    parser.add_argument("--entry-line", type=str, default=None,
                        help="Entry line segment as x1,y1,x2,y2 in percentages "
                             ...
                             "Default: 30,0,30,100")
    parser.add_argument("--capture-line", type=str, default=None,
                        help="Capture line segment as x1,y1,x2,y2 in percentages "
                             "(0-100) or fractions (0.0-1.0). "
                             "Default: 55,0,55,100")
...
class CentroidTracker:
    ...
    def __init__(self, max_dist=MAX_MATCH_DIST, max_disappeared=MAX_DISAPPEARED):
        ...
        self.crossed_capture = OrderedDict() # id -> True if crossed capture line
    ...
    def _register(self, cx, cy, bbox):
        ...
        self.crossed_capture[oid] = False
    ...
    def check_line_crossings(self, entry_line, capture_line, exit_line):
        """
        Check line crossings this frame.
        Returns (new_capture_ids, new_count_ids).
          new_capture_ids: objects that just crossed the capture line (snapshot now)
          new_count_ids:   objects that just crossed the exit line (save + count)
        """
        new_capture_ids = []
        new_count_ids = []
        ...
        for oid in list(self.objects.keys()):
            ...
            if self.crossed_entry[oid] and not self.crossed_capture[oid]:
                if segments_intersect(prev_pos, current_pos, capture_line[0], capture_line[1]):
                    self.crossed_capture[oid] = True
                    new_capture_ids.append(oid)

            if self.crossed_capture[oid] and not self.counted[oid]:
                if segments_intersect(prev_pos, current_pos, exit_line[0], exit_line[1]):
                    self.counted[oid] = True
                    new_count_ids.append(oid)

        return new_capture_ids, new_count_ids
...
def main():
    ...
    try:
        ...
        capture_line_fracs = parse_line_segment(args.capture_line,  LINE_CAPTURE_FRAC)
    ...
    capture_line = line_to_pixels(capture_line_fracs, W, H)
    ...
    print(f"Capture line : {capture_line[0]} -> {capture_line[1]}")
    ...
    capture_crops    = {}  # oid -> best crop seen before capture line (in memory)
    best_crop_area   = {}  # oid -> bbox area of best crop stored so far
    debug_mode = False
    prev_mask = None
    frame_n = 0
    ...
    while True:
        ...
        # Continuously update best (largest bbox) crop for objects between entry and capture
        for oid in list(tracker.objects.keys()):
            if tracker.crossed_entry[oid] and not tracker.crossed_capture[oid]:
                bbox = tracker.boxes.get(oid)
                if bbox is not None:
                    area = bbox[2] * bbox[3]
                    if area > best_crop_area.get(oid, 0):
                        crop = crop_object_image(frame, bbox)
                        if crop is not None:
                            capture_crops[oid] = crop
                            best_crop_area[oid] = area

        # Check line crossings
        new_capture_ids, new_count_ids = tracker.check_line_crossings(
            entry_line, capture_line, exit_line)

        # At capture line: best crop is already stored — just log it
        for oid in new_capture_ids:
            if oid in capture_crops:
                print(f"[CAPTURE] Track #{oid} best crop locked at frame {frame_n} "
                      f"(area={best_crop_area.get(oid, 0)}px²)")

        # At exit line: save the captured snapshot + confirmed/unconfirmed
        count += len(new_count_ids)
        if new_count_ids:
            print(f"[COUNT {count}] +{len(new_count_ids)} object(s) crossed entry->capture->exit at frame {frame_n}")

        for oid in new_count_ids:
            bbox = tracker.boxes.get(oid)
            _captured = capture_crops.pop(oid, None)
            crop = _captured if _captured is not None else crop_object_image(frame, bbox)
        ...
        line2 = (f"FPS  src: {fps:.1f}    compute: {compute_fps:.1f}    "
                 f"loop: {loop_fps:.1f}    "
                 f"[ENTRY=yellow  CAPTURE=magenta  EXIT=blue]")
